    ...
    def __iter__(self):
        if len(self.indices) > 1:
            ...
            for _ in range(self.env_len):
                for i in range(len(l_indices)):
                    l_iter.extend(l_indices[i][:1])
                    del l_indices[i][:1]
            
        else:
            l_iter = copy.deepcopy(self.indices[0])
            random.shuffle(l_iter)
...
def network_features(adj, traj):
    
    eigf_calls = EigenFeatures(mode='all')

    cycle_calls = NodeCycleFeatures()

    right_1 = torch.roll(traj, shifts=1, dims=1)
    right_2 = torch.roll(traj, shifts=2, dims=1)
    right_1[:, 0] = 0
    right_2[:, 0:2] = 0

    d_traj_hi = traj - right_1
    d2_traj_hi = traj - 2 * right_1 + right_2

    d_traj_hi[:, 0] = 0
    d2_traj_hi[:, 0:2] = 0

    traj = traj.unsqueeze(-1)
    d_traj_hi = d_traj_hi.unsqueeze(-1)
    d2_traj_hi = d2_traj_hi.unsqueeze(-1)

    fused_traj = torch.cat([traj, d_traj_hi, d2_traj_hi], dim=-1)
...
class EigenFeatures:
    ...
    def __call__(self, A):
        ...
        if self.mode == 'eigenvalues':
            ...
            return n_connected_comp, batch_eigenvalues, nonlcc_indicator, k_lowest_eigenvector, beta.unsqueeze(-1)
        else:
...
def get_eigenvectors_features(vectors, n_connected, k=2):
    """
    ...
    first_ev = torch.round(vectors[:, :, 0], decimals=3)                        # bs, n
    # Add random value to the mask to prevent 0 from becoming the mode
    random = torch.randn(bs, n, device=vectors.device)                          # bs, n
    ...
    not_lcc_indicator = (mask).unsqueeze(-1).float()
...
def calc_beta(A):
    '''
    Input A: batched adjacency matrix (bs, n, n)
    output beta: scalar
    ''' 
    denominator = torch.sum(torch.sum(A, dim=-1), dim=-1)
    molecular = torch.sum(torch.sum(torch.matmul(A, A), dim=-1), dim=-1)
    # consider the position where denominator is 0, if denominator is 0, then beta is 0
    beta = molecular/(denominator+1e-12)
    beta[denominator == 1e-12] = 0
    return beta
