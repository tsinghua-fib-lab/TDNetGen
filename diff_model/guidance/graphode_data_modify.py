...
import numpy as np
import utils
from diff_model.diffusion import diffusion_utils
...
from guidance.model_libs.odeblock import *
from guidance.utils import *
from guidance.model_libs.classifier import *
...
class GraphODEClassifier(pl.LightningModule):
    ...
    #     for i in range(specs['num_classifier_layers']):
    #         if i == specs['num_classifier_layers'] - 1:
    #             classifier.add_module(f'fc-{i}',
    #                                   nn.Linear(specs['node_dim'], specs['clasf_out_dim']))
    #             classifier.add_module('sigmoid', nn.Sigmoid())
    #         elif i == 0:
    #             classifier.add_module(f'fc-{i}',
    #                                     nn.Linear(specs['num_traj'] + 1, specs['node_dim']))
    #             classifier.add_module('relu', nn.ReLU())
    #         else:
    #             classifier.add_module(f'fc-{i}',
    #                                     nn.Linear(specs['node_dim'], specs['node_dim']))
    #             classifier.add_module('relu', nn.ReLU())
    ...
    def transfer_to_adjacent(self, X: th.Tensor, E: th.Tensor):
        ...
        # check_mat = th.tensor([0, 1]).repeat(*target_shape[:3], 1).to(E.device)
        # A = th.mul(E, check_mat).sum(-1)
        return A
    ...
    def model_forward(self, X, E, t, node_mask, noisy_data):
        ...
        B, N = E.shape[0], E.shape[1]
        traj_min = self.config.train['traj_min']
        traj_max = self.config.train['traj_max']

        x_0_lo = th.rand((B, N, self.time_ticks, 1)).to(E.device)
        x_0_lo = (x_0_lo - traj_min) / (traj_max - traj_min)
        x_0_lo_feat = th.zeros((B, N, self.time_ticks, 2)).to(E.device)
        x_0_lo = th.cat([x_0_lo, x_0_lo_feat], dim=-1)

        x_0_hi = th.rand((B, N, self.time_ticks, 1)).to(E.device) * 5
        x_0_hi = (x_0_hi - traj_min) / (traj_max - traj_min)
        x_0_hi_feat = th.zeros((B, N, self.time_ticks, 2)).to(E.device)
        ...
        x_hat_lo_clasf_input = x_hat_lo[:, :, -self.classification_steps:]
        x_hat_hi_clasf_input = x_hat_hi[:, :, -self.classification_steps:]
    ...
    def unwrapped_forward(self, noisy_data, extra_data, node_mask, t_eval, x):
        ...
        # num_stable_steps = th.count_nonzero(th.ge(t_eval, t_eval[-1] - self.stable_window))
        num_stable_steps = self.stable_window
        # print(num_stable_steps)
        X = noisy_data['X_t']
        E = noisy_data['E_t']
        t = noisy_data['t']
        y = noisy_data['y_t']
        B, N = E.shape[0], E.shape[1]
        # x_0_lo = th.zeros((B, N, 1)).to(E.device)
        # x_0_hi = th.zeros((B, N, 1)).to(E.device) + 5

        # x_hat_hi = self.forward(x['hi'].to(th.float32).unsqueeze(-1), X, E, t_eval) # (B, N, T)
        # x_hat_lo = self.forward(x['lo'].to(th.float32).unsqueeze(-1), X, E, t_eval) # (B, N, T)
        ...
        if self.config.general.clasf_resi:
            x_hat_lo_clasf_input = x_hat_lo[:, :, -self.classification_steps:]
            x_hat_hi_clasf_input = x_hat_hi[:, :, -self.classification_steps:]
            ...
            # x_hi_indi = x_hat_hi[:, :, -1].mean(dim=1,keepdim=True)
            # x_lo_indi = x_hat_lo[:, :, -1].mean(dim=1,keepdim=True)

            # clasf_input = th.cat([x_hi_indi, x_lo_indi, t], dim=1)

            # y_hat = self.classifier(clasf_input)
            ...
            if self.training:
                ...
                try:
                    # resi_aucs = roc_auc_score(y.cpu().numpy(), y_hat.cpu().numpy())
        ...
        # print((x['hi'].reshape(-1, self.time_ticks)[:, :-num_stable_steps]))
        # assert False

        if len(x['lo'].shape) > 3:

            dynamic_loss = self.loss(x_hat_lo.reshape(-1, self.time_ticks)[:, :-num_stable_steps], x['lo'][..., 0].reshape(-1, self.time_ticks)[:, :-num_stable_steps]) + \
    ...
    # def unwrapped_forward(self, batch):
    #     x, adj, t_eval, batch_indices = \
    #         batch.x, batch.adj, batch.t_eval, batch.batch
    #     t_eval = t_eval[0]
    #     num_stable_steps = th.count_nonzero(th.ge(t_eval, t_eval[-1] - self.stable_window))
    #     x_hat = self.forward(x[:, 0].unsqueeze(-1), adj, t_eval)
    #     dynamic_loss = self.loss(x_hat[:, :-num_stable_steps], x[:, :-num_stable_steps])
    #     stable_loss = self.loss(x_hat[:, -num_stable_steps:], x[:, -num_stable_steps:])
    #     loss = dynamic_loss + self.stable_coef * stable_loss
        # relative_error = th.mean(
        #     th.abs(x_hat - x) / th.where(th.gt(th.abs(x), self.EPSILON), th.abs(x), th.ones_like(x)))
    #     dynamic_relative_error = th.mean(
    #         th.abs(x_hat[:, :-num_stable_steps] - x[:, :-num_stable_steps]) /
    #         th.where(th.gt(th.abs(x[:, :-num_stable_steps]), self.EPSILON),
    #                  th.abs(x[:, :-num_stable_steps]),
    #                  th.ones_like(x[:, :-num_stable_steps])))
    #     stable_relative_error = th.mean(
    #         th.abs(x_hat[:, -num_stable_steps:] - x[:, -num_stable_steps:]) /
    #         th.where(th.gt(th.abs(x[:, -num_stable_steps:]), self.EPSILON),
    #                  th.abs(x[:, -num_stable_steps:]),
    ...
    #     return loss, dynamic_loss, stable_loss, relative_error, \
    #         dynamic_relative_error, stable_relative_error, num_graphs
    ...
    def training_step(self, data, i):
        ...
        self.is_test = False
        # loss, dynamic_loss, stable_loss, relative_error, dynamic_relative_error, stable_relative_error = self.unwraaped_forward()
        # loss, dynamic_loss, stable_loss, relative_error, dynamic_relative_error, stable_relative_error, batch_size = \
        #     self.unwrapped_forward(batch)
        # loss, dynamic_loss, stable_loss, relative_error, dynamic_relative_error, stable_relative_error = self.unwraaped_forward
        ...
        loss, dynamic_loss, stable_loss, resilience_loss, relative_error, dynamic_relative_error, stable_relative_error, resi_metrics,_ = \
            self.unwrapped_forward(noisy_data, extra_data, node_mask, t_eval, noisy_data['real_traj'])
    ...
    def validation_step(self, data, i):
        ...
        extra_data = self.compute_extra_data(noisy_data)
        batch_size = X.shape[0]
        loss, dynamic_loss, stable_loss, resilience_loss, relative_error, dynamic_relative_error, stable_relative_error, resi_metrics, _ = self.unwrapped_forward(noisy_data, extra_data, node_mask, t_eval, noisy_data['real_traj'])
