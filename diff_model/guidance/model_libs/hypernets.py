...
from diff_model.guidance.model_libs.odeblock import ODEBlock
    ...
    def update_ghost(self):
        ...
        if 'bipartite' in self.specs:
            if self.specs['bipartite']:
                self.nets["ghost"].ode_func.update_top_nodes(self.nets['ghost_structure'].ode_func.top_nodes,self.nets['ghost_structure'].ode_func.all_nodes_num,self.nets['ghost_structure'].ode_func.node_mask)
...
class HyperGODE(nn.Module):
    ...
    def update_ghost(self, X_in, E_in, node_mask, top_nodes=None):

        all_nodes_num = node_mask.sum(dim=-1)
        ...
        if 'bipartite' in self.specs:
            ...
            # assert False
            if self.specs['bipartite']:
                self.net_root.ode_func.update_top_nodes(top_nodes, all_nodes_num, node_mask)
                self.ghost_structure.ode_func.update_top_nodes(top_nodes, all_nodes_num, node_mask)
        # else:
        #     print('bipartite not in!!!!!')
        #     assert False
