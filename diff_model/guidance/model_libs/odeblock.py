...
from diff_model.guidance.model_libs.base_modules import GCNConv, GroupGCNConv
from diff_model.guidance.gdlibs.utils import *
...
class GroupODEFunc(nn.Module):
    ...
    def update_top_nodes(self, top_nodes, all_nodes_num,node_mask) -> None:
    ...
    def forward(self, t, x):  # How to use t?
        ...
        # th.save(A, 'inter_A.pt')
        # th.save(self.top_nodes, 'inter_top_nodes.pt')

        if 'bipartite' in self.config:
            if self.config.bipartite:
                A = trans_bipartite_to_weighted_block(A, self.top_nodes, self.node_mask, self.all_nodes_num)
