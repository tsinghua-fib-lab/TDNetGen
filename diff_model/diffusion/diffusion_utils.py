...
from diff_model.utils import PlaceHolder
...
def create_edge_mask_from_node_mask(node_mask):
    ...
    # 创建 edge_mask 为全 False
    node_mask = node_mask.cpu().numpy()
    edge_mask = np.zeros((B, N, N), dtype=bool)

    # 找到每一行中第一个1的位置和0再次出现的位置
    first_one_pos = np.argmax(node_mask, axis=1)
    mask_one_present = node_mask[np.arange(B), first_one_pos] == 1  # 检查是否真的存在1

    # 如果一行的最后一个元素是1，那么认为这一行中的1之后没有0出现
    # 如果1之后有0出现，找到第一个0的位置
    first_zero_after_one = np.zeros(B, dtype=int)
    for i in range(B):
        if mask_one_present[i]:
            first_zero_after_one[i] = np.argmax(node_mask[i, first_one_pos[i]:] == 0) + first_one_pos[i]
            # 如果这一行的1之后没有0，将位置设置为N
            if first_zero_after_one[i] == first_one_pos[i]:
                first_zero_after_one[i] = N
        else:
            first_zero_after_one[i] = N

    # 使用 NumPy 广播和切片填充 edge_mask
    for b in range(B):
        x = first_one_pos[b]
        y = first_zero_after_one[b]
        edge_mask[b, :x, x:y] = True  # 前 N-x 行，x 列到 y-1 列为 True
        edge_mask[b, x:y, :x] = True  # 后 x 行，x 列之前的列为 True

    return edge_mask

def sample_discrete_features(probX, probE, node_mask, bipart_mask=None):
    ...
    if bipart_mask is not None:
        edge_mask = create_edge_mask_from_node_mask(bipart_mask)
        edge_mask = torch.from_numpy(edge_mask).to(probE.device)
...
def sample_discrete_feature_noise(limit_dist, node_mask,bipartite_mask=None):
    ...
    if bipartite_mask is not None:
        ...
        edge_mask = create_edge_mask_from_node_mask(bipartite_mask)
        edge_mask = torch.from_numpy(edge_mask).to(U_E.device)
        edge_mask = edge_mask.unsqueeze(-1).type_as(U_E).expand_as(U_E)
        replacement = torch.tensor([1, 0], dtype=U_E.dtype, device=U_E.device).repeat(bs, n_max, n_max, 1)
        U_E = torch.where(edge_mask == 1, U_E, replacement)
        
    assert (U_E == torch.transpose(U_E, 1, 2)).all()

    return PlaceHolder(X=U_X, E=U_E, y=U_y).mask(node_mask)

def sample_padding_feature(padding_dist, node_mask, bipartite_mask=None):
    bs, n_max = node_mask.shape
    x_padding = padding_dist.X[None, None, :].expand(bs, n_max, -1)
    e_padding = padding_dist.E[None, None, None, :].expand(bs, n_max, n_max, -1)
    y_padding = padding_dist.y[None, :].expand(bs, -1)
    U_X = x_padding.flatten(end_dim=-2).multinomial(1).reshape(bs, n_max)
    U_E = e_padding.flatten(end_dim=-2).multinomial(1).reshape(bs, n_max, n_max)
    U_y = torch.empty((bs, 0))
    ...
    U_X = F.one_hot(U_X, num_classes=x_padding.shape[-1]).float()
    U_E = F.one_hot(U_E, num_classes=e_padding.shape[-1]).float()
    ...
    return PlaceHolder(X=U_X, E=U_E, y=U_y)
