...
from diff_model.analysis.dist_helper import compute_mmd, gaussian_emd, gaussian, emd, gaussian_tv, disc
...
def resi_rate_stats(graph_ref_list, graph_pred_list, beta_c, is_parallel=True, compute_emd=False):
    ...
    if is_parallel:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for beta in executor.map(calc_beta_worker, graph_pred_list_remove_empty, repeat(beta_c)):
                sample_pred.append(beta)
    else:
       
        for i in range(len(graph_pred_list_remove_empty)):
            beta_temp = calc_beta_worker(graph_pred_list_remove_empty[i], beta_c)
            sample_pred.append(beta_temp)
    ...
    return sample_pred.sum()/sample_pred.shape[0]
...
def resi_rate_bi_stats(graph_ref_list, graph_pred_list, beta_c, tns, is_parallel=True, compute_emd=False):
    ...
    sample_pred = []
    # in case an empty graph is generated
    graph_pred_list_remove_empty = [(G, tn) for G, tn in zip(graph_pred_list,tns) if not G.number_of_nodes() == 0]

    mapped_graphs = []

    for graph, tn in graph_pred_list_remove_empty:
        adjacency = nx.to_numpy_array(graph)

        bipartite_adj = adjacency[:tn, tn:]

        A, B = create_weighted_networks(bipartite_adj)
        
        mapped_graphs.append(nx.from_numpy_array(A))
        mapped_graphs.append(nx.from_numpy_array(B))

    prev = datetime.now()
    if is_parallel:
        
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for beta in executor.map(calc_beta_worker, mapped_graphs, repeat(beta_c)):
                sample_pred.append(beta)
    else:
    
        for i in range(len(mapped_graphs)):
            beta_temp = calc_beta_worker(mapped_graphs[i], beta_c)
            sample_pred.append(beta_temp)
...
def node_stats(graph_ref_list, graph_pred_list, is_parallel=True, compute_emd = False):
    ...
    if is_parallel:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for node in executor.map(node_worker, graph_ref_list):
                sample_ref.append(node)
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for node in executor.map(node_worker, graph_pred_list_remove_empty):
                sample_pred.append(node)
    else:
        for i in range(len(graph_ref_list)):
            node_temp = node_worker(graph_ref_list[i])
            sample_ref.append(node_temp)
        for i in range(len(graph_pred_list_remove_empty)):
            node_temp = node_worker(graph_pred_list_remove_empty[i])
            sample_pred.append(node_temp)
    

    wd = wasserstein_distance(sample_pred, sample_ref)

    mean_distance = np.abs(np.array(sample_ref).mean() - np.array(sample_pred).mean())

    elapsed = datetime.now() - prev
    if PRINT_TIME:
        print('Time computing degree mmd: ', elapsed)
    
    return wd, mean_distance
...
def edge_stats(graph_ref_list, graph_pred_list, is_parallel=True, compute_emd = False):
    ...
    if is_parallel:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for edge in executor.map(edge_worker, graph_ref_list):
                sample_ref.append(edge)
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for edge in executor.map(edge_worker, graph_pred_list_remove_empty):
                sample_pred.append(edge)
    else:
        for i in range(len(graph_ref_list)):
            edge_temp = edge_worker(graph_ref_list[i])
            sample_ref.append(edge_temp)
        for i in range(len(graph_pred_list_remove_empty)):
            edge_temp = edge_worker(graph_pred_list_remove_empty[i])
            sample_pred.append(edge_temp)
...
def beta_stats(graph_ref_list, graph_pred_list, is_parallel=True, compute_emd = False):
    ...
    if is_parallel:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for beta in executor.map(beta_worker, graph_ref_list):
                sample_ref.append(beta)
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for beta in executor.map(beta_worker, graph_pred_list_remove_empty):
                sample_pred.append(beta)
    else:
        for i in range(len(graph_ref_list)):
            beta_temp = beta_worker(graph_ref_list[i])
            sample_ref.append(beta_temp)
        for i in range(len(graph_pred_list_remove_empty)):
            beta_temp = beta_worker(graph_pred_list_remove_empty[i])
            sample_pred.append(beta_temp)
...
class SpectreSamplingMetrics(nn.Module):
    def __init__(self,dataloaders, compute_emd, metrics_list, beta_c=None, dataloaders_cond=None):
        super().__init__()
        ...
        self.beta_c = beta_c
        self.dataloaders_cond = dataloaders_cond
        torch.save(self.train_graphs, 'graph_adjs/train_graphs.pt')
        torch.save(self.val_graphs, 'graph_adjs/val_graphs.pt')
        torch.save(self.test_graphs, 'graph_adjs/test_graphs.pt')
    def loader_to_nx(self, loader):
        networkx_graphs = []
        for i, batch in enumerate(loader):
    ...
    def forward(self, generated_graphs: list, name, current_epoch, val_counter, save_graphs=True, test=False,comm=False,bipartite=False):
        print(f"Computing sampling metrics between {len(generated_graphs)} generated graphs and {len(self.test_graphs)}"
              f" test graphs -- emd computation: {self.compute_emd}")
        
        if test:
            ref_graphs = self.test_graphs
        else:
            ref_graphs = self.val_graphs
        networkx_graphs = []
        adjacency_matrices = []
        tns = []
        print("Building networkx graphs...")
        for graph in generated_graphs:
            if comm:
                node_types, edge_types, _ = graph
            elif bipartite:
                node_types, edge_types, tn = graph
            else:
                if len(graph) == 2:
                    node_types, edge_types = graph
                else:
                    node_types, edge_types, _, __ = graph
            A = edge_types.bool().cpu().numpy()
            adjacency_matrices.append(A)
            if bipartite:
                tns.append(tn)
        ...
        if 'degree' in self.metrics_list:
            print("Computing degree stats..")
            degree = degree_stats(ref_graphs, networkx_graphs, is_parallel=True,
                                  compute_emd=self.compute_emd)
            # wandb.run.summary['degree'] = degree
        ...
        if 'spectre' in self.metrics_list:
            print("Computing spectre stats...")
            spectre = spectral_stats(ref_graphs, networkx_graphs, is_parallel=True, n_eigvals=-1,
                                     compute_emd=self.compute_emd)
        ...
        if 'clustering' in self.metrics_list:
            print("Computing clustering stats...")
            clustering = clustering_stats(ref_graphs, networkx_graphs, bins=100, is_parallel=True,
                                          compute_emd=self.compute_emd)
        ...
        if 'motif' in self.metrics_list:
            ...
            motif = motif_stats(ref_graphs, networkx_graphs, motif_type='4cycle', ground_truth_match=None, bins=100,
                                compute_emd=self.compute_emd)
            to_log['motif'] = motif
            # wandb.run.summary['motif'] = motif
        ...
        if 'orbit' in self.metrics_list:
            print("Computing orbit stats...")
            orbit = orbit_stats_all(ref_graphs, networkx_graphs, compute_emd=self.compute_emd)
            to_log['orbit'] = orbit
            # wandb.run.summary['orbit'] = orbit

        if 'resi_rate' in self.metrics_list:
            print("Computing resilience rate...")
            resi_rate = resi_rate_stats(ref_graphs, networkx_graphs, beta_c = self.beta_c, is_parallel=True, compute_emd=self.compute_emd)
            to_log['resi_rate'] = resi_rate

        if 'resi_rate_bi' in self.metrics_list:
            print("Computing Bipartite resilient rate...")
            resi_rate_bi = resi_rate_bi_stats(ref_graphs, networkx_graphs, beta_c=self.beta_c, tns=tns, is_parallel=True, compute_emd=self.compute_emd)
            to_log['resi_rate_bi'] = resi_rate_bi

        if 'node' in  self.metrics_list:
            print("Computing node number stats...")
            wd_divergence, node_divergence = node_stats(ref_graphs, networkx_graphs, is_parallel=True,
                                                compute_emd=self.compute_emd)
            to_log['node_wd'] = wd_divergence
            to_log['node_mean'] = node_divergence
        
        if 'edge' in self.metrics_list:
            print("Computing edge number stats...")
            wd_divergence, edge_divergence = edge_stats(ref_graphs, networkx_graphs, is_parallel=True,
                                                compute_emd=self.compute_emd)
            to_log['edge_wd'] = wd_divergence
            to_log['edge_mean'] = edge_divergence

        if 'beta' in self.metrics_list:
            print("Computing beta stats...")
            wd_divergence, beta_divergence = beta_stats(ref_graphs, networkx_graphs, is_parallel=True,
                                                compute_emd=self.compute_emd)
            to_log['beta_wd'] = wd_divergence
            to_log['beta_mean'] = beta_divergence
...
class ResiSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, dataloaders, beta_c, dataloaders_cond=None):
        super().__init__(dataloaders=dataloaders,
                         compute_emd=False,
                         metrics_list=['resi_rate', 'degree', 'clustering', 'orbit', 'spectre', 'node', 'edge', 'beta', 'vun'],beta_c=beta_c, dataloaders_cond=dataloaders_cond)

class ResiBipartiteSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, dataloaders, beta_c, dataloaders_cond=None):
        super().__init__(dataloaders=dataloaders,
                         compute_emd=False,
                         metrics_list=['resi_rate_bi', 'degree', 'clustering', 'orbit', 'spectre', 'node', 'edge', 'beta', 'vun'],beta_c=beta_c, dataloaders_cond=dataloaders_cond)