...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
                 domain_features, guidance_model=None, load_model=None):
        super().__init__()
        ...
        self.lambda_guidance = cfg.general.lambda_guidance

        self.guidance_model = guidance_model
        self.use_guidance = cfg.general.use_guidance

        self.guidance_direction = cfg.general.guidance_direction
    ...
    def test_step(self, data, i, dataloader_idx=0):
        ...
        if self.conditional:
            if dataloader_idx == 0:
                ...
                return {'loss': 0, 'gt_data': dense_data, 'ori_data':ori_data}
    ...
    def test_epoch_end(self, outs) -> None:
        ...
        full_dict = self.transfer_dataset_form(samples)

        if self.guidance_direction == 1:

            torch.save(full_dict, 'graph_adjs/enhance_pos.pt')

        elif self.guidance_direction == 0:

            torch.save(full_dict, 'graph_adjs/enhance_neg.pt')
    ...
    def transfer_dataset_form(self, samples):
        ...
        for i in range(len(samples)):
            As_use.append(samples[i][1])
            x_reals_hi_use.append(samples[i][2])
            x_reals_lo_use.append(samples[i][3])
            labels_use.append(direct)
            ts_use.append(0)
    

        full_dict = {'As': As_use, 'x_reals_his': x_reals_hi_use, 'x_reals_los': x_reals_lo_use, 'labels': labels_use, 'ts': ts_use}
    ...
    def sample_batch(self, batch_id: int, batch_size: int, keep_chain: int, number_chain_steps: int,
        ...
        if self.conditional:
            ...
            if self.cfg.model.resample:
                ...
                for s_int in tqdm(reversed(range(0, self.T)), total=len(range(0, self.T))):
                    ...
                    if self.cfg.general.save_intermediate:
                        if s_int % 25 == 0:
                            sampled_s_save = copy.deepcopy(sampled_s)
                            sampled_s_Save = sampled_s_save.mask(node_mask, collapse=True)
                            ...
                            torch.save(molecule_list, f'./intermediate_graphs/sampled_network_{batch_id}_{s_int}.pt')
                    if s_int == 0:
                        sampled_s.E = E
                # Sample
            ...
            # Sample
            sampled_s = sampled_s.mask(node_mask, collapse=True)
            X, E, y, traj_hi, traj_lo = sampled_s.X, sampled_s.E, sampled_s.y, sampled_s.traj_hi, sampled_s.traj_lo
        ...
        for i in range(batch_size):
            ...
            here_traj_hi = traj_hi[i, :n].cpu()
            here_traj_lo = traj_lo[i, :n].cpu()
            if self.cfg.general.control_strategy == 'community':
                rt = control_rate[i].cpu().item()
                molecule_list.append([atom_types, edge_types, rt, here_traj_hi, here_traj_lo])
            else:
                molecule_list.append([atom_types, edge_types, here_traj_hi, here_traj_lo])
        ...
        for i in range(batch_size):
            ...
            if self.cfg.general.control_strategy == 'community':
                rt = control_rate[i].cpu().item()
                predicted_graph_list.append([atom_types, edge_types,rt, here_traj_hi, here_traj_lo])
            else:
                predicted_graph_list.append([atom_types, edge_types, here_traj_hi, here_traj_lo])
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask, input_properties=None):
        ...
        if self.use_guidance:
            ...
            lamb = self.lambda_guidance
            if self.guidance_direction == 1:
                input_properties = torch.ones(noisy_data['X_t'].shape[0], 1)

            elif self.guidance_direction == 0:

                input_properties = torch.zeros(noisy_data['X_t'].shape[0], 1)
            ...
            grad_x, grad_e, traj_hi, traj_lo = self.cond_fn(noisy_data, node_mask, input_properties)
            ...
            p_eta_e = p_eta_e.nan_to_num(0.5)

            # torch.save(p_eta_e, 'grad_e.pt')
            
            # torch.save(p_eta_e, './p_eta_e.pt')
            # torch.save(E_t, './e_t.pt')
            # torch.save(node_mask, './node_mask.pt')
            # assert False
        ...
        out_one_hot = utils.PlaceHolder(X=X_s, E=E_s, y=torch.zeros(y_t.shape[0], 0), traj_hi = traj_hi, traj_lo = traj_lo)
        out_discrete = utils.PlaceHolder(X=X_s, E=E_s, y=torch.zeros(y_t.shape[0], 0), traj_hi = traj_hi, traj_lo = traj_lo)
    ...
    def cond_fn(self, noisy_data, node_mask, target=None):
        ...
        with torch.enable_grad():
            ...
            y = y.float().detach().requires_grad_(True)
            node_mask = node_mask.float().detach().requires_grad_(True)

            # pred = self.guidance_model.model(x_in, e_in, y, node_mask)
            

            pred, traj_hi, traj_lo = self.guidance_model.model_forward(x_in, e_in, y, node_mask, noisy_data)
            ...
            # mse = loss(pred, target.repeat(pred.size(0), 1))
            mse = loss(pred, target)


            t_int = int(t[0].item() * 500)
            if t_int % 10 == 0:
                print(f'Cross Entropy at step {t_int}: {mse.item()}')
            self.log('Guidance CE', mse.item(), on_step=False, on_epoch=True, prog_bar=True, logger=True)
            # calculate gradient of mse with respect to x and e

            # grad_x = torch.autograd.grad(mse, x_in, retain_graph=True)[0]
            grad_x = torch.ones_like(x_in)
            grad_e = torch.autograd.grad(mse, e_in)[0]
            # mse.backward()

            torch.save(grad_e, 'grad_e.pt')
            ...
            return mask_grad_x, mask_grad_e, traj_hi, traj_lo