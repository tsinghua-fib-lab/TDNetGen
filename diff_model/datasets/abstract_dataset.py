from diff_model.diffusion.distributions import DistributionNodes
import diff_model.utils as utils
import torch
...
class AbstractDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self, datasets, datasets_cond=None) -> None:
    ...
    def top_node_counts(self, max_nodes_possible=300):
        all_counts = torch.zeros(max_nodes_possible)
        for split in ['train', 'val', 'test']:
            for i, data in enumerate(self.dataloaders[split]):
                uniques, counts = torch.unique(data.t_nodes, return_counts=True)
                for unique, count in zip(uniques, counts):
                    all_counts[unique] += count
        max_index = max(all_counts.nonzero())
        all_counts = all_counts[:max_index + 1]
        all_counts = all_counts / all_counts.sum()
        return all_counts
...
class AbstractDatasetInfos:
    def complete_infos(self, n_nodes, node_types, n_nodes_val=None, n_nodes_test=None,n_nodes_top=None):
        self.input_dims = None
        self.output_dims = None
        ...
        if n_nodes_top is not None:
            self.top_dist = DistributionNodes(n_nodes_top)
